...
class FOVWindow(Window):

    name = 'FOV'

    def __init__(self, main, useless=0,
            tab_id=3):
        ...
        self.add_side_widget(self.showRoisB)

        # self.roiIdB= QtWidgets.QCheckBox('with ID #')
        # self.add_side_widget(self.roiIdB)
        ...
        self.add_side_widget(self.preprocessB)

        self.greenBox = QtWidgets.QCheckBox("green ROIs")
        self.add_side_widget(self.greenBox)
        self.greenBox.setChecked(True)

        self.redBox = QtWidgets.QCheckBox("red ROIs")
        self.add_side_widget(self.redBox)
        self.redBox.setChecked(True)

        self.add_side_widget(QtWidgets.QLabel('Image:'))
        self.imgB = QtWidgets.QComboBox(self.main)
        self.imgB.addItems(KEYS)
        self.imgB.activated.connect(self.draw_image_FOV)
        self.add_side_widget(self.imgB)

        # self.add_side_widget(# QtWidgets.QLabel('Display Exponent:'), 'large-left')
        self.expT= QtWidgets.QDoubleSpinBox(self.main)
        self.expT.setValue(0.25)
        self.expT.setSuffix(' (display exponent)')
        self.add_side_widget(self.expT)#, 'small-right')

        self.sizeT= QtWidgets.QDoubleSpinBox(self.main)
        self.sizeT.setValue(2)
        self.sizeT.setSuffix(' (ROI size)')
        self.add_side_widget(self.sizeT)#, 'small-right')
    ...
    # ----------------------------------------------------------
    on_refresh = draw_image_FOV
    on_process = prev_ROI_FOV
    on_toggle = toggle_FOV
    on_next = next_ROI_FOV
