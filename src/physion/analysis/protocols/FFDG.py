...
def generate_pdf(nwbfile,
    ...
    LOCS = [(200, 70),
            (200, 550), (200, 1300), (200, 2000), (200, 2700),
            (800, 90)]
    ...
    LOCS = [(1500, 200), (100, 600)]

    for i in rois:

        KEYS.append('TA-%i'%i)
        LOCS.append((100, 1200+500*i))
    ...
    cmd = '/usr/bin/pdftk %s %s cat output %s' % (os.path.join(tempfile.tempdir,
                                                               'session-summary-1.pdf'),
                                                  os.path.join(tempfile.tempdir,
                                                                'session-summary-2.pdf'),
                                                  pdf_file)
...
def generate_figs(nwbfile,
    ...
    # ##  --- FOVs ---
    fig, AX = pt.plt.subplots(1, 3, figsize=(4,1.5))
    ...
    tlim = [data.tlim[1]/2.-90, data.tlim[1]/2.-60]
    fig, ax = pt.plt.subplots(1, figsize=(7, 2.5))
    pt.plt.subplots_adjust(bottom=0, top=0.9, left=0.05, right=0.95)
    ax.annotate('t=%.1fmin  ' % (tlim[1]/60), (1,1), 
                 ha='right', xycoords='axes fraction', size=8)
    ...
    tlim = [data.tlim[1]-90, data.tlim[1]-60]
    fig, ax = pt.plt.subplots(1, figsize=(7, 2.5))
    pt.plt.subplots_adjust(bottom=0, top=0.9, left=0.05, right=0.95)
    ax.annotate('t=%.1fmin  ' % (tlim[1]/60), (1,1), 
                 ha='right', xycoords='axes fraction', size=8)
