...
class TrialAveragingWindow(Window):

    name = 'trial_averaging'

    def __init__(self, main,
                        box_width=250,
                        tab_id=2):

        if main.data is None:
            main.statusBar.showMessage(' [!!] need to load a NWB file first [!!] ')
            return
        ...
        self.pbox = QtWidgets.QComboBox(self.main)
        self.pbox.addItem('')
        ...
        self.add_side_widget(self.guiKeywords)
        ...
        self.add_side_widget(self.roiPickTA)

        self.prevBtn = QtWidgets.QPushButton('[P]rev', self.main)
        self.add_side_widget(self.prevBtn, 'small-left')
        self.prevBtn.clicked.connect(self.prev_ROI_TA)
        self.nextBtn = QtWidgets.QPushButton('[N]ext roi', self.main)
        self.nextBtn.clicked.connect(self.next_ROI_TA)
        self.add_side_widget(self.nextBtn, 'large-right')
    
        self.computeBtn = QtWidgets.QPushButton('[C]ompute episodes', self.main)
        self.computeBtn.setMaximumWidth(box_width)
        self.computeBtn.clicked.connect(self.compute_episodes)
        self.add_side_widget(self.computeBtn)

        # # then parameters
        self.add_side_widget(QtWidgets.QLabel(\
                7*'-'+' Display options '+7*'-', self.main))

        for i in range(NMAX_PARAMS): 
            setattr(self, "box%i"%i, QtWidgets.QComboBox(self.main))
            getattr(self, "box%i"%i).setMaximumWidth(box_width)

            self.add_side_widget(getattr(self, "box%i"%i))

        self.refreshBtn = QtWidgets.QPushButton('[R]efresh plots', self.main)
        self.refreshBtn.setMaximumWidth(box_width)
        self.refreshBtn.clicked.connect(self.refresh_TA)
        self.add_side_widget(self.refreshBtn)
    
        self.allBtn = QtWidgets.QPushButton('[F]=[N]+[C]+[R]', self.main)
        self.allBtn.setMaximumWidth(box_width)
        self.allBtn.clicked.connect(self.next_and_plot_TA)
        self.add_side_widget(self.allBtn)

        self.samplingBox = QtWidgets.QDoubleSpinBox(self.main)
        ...
        self.add_side_widget(self.samplingBox)
    ...
    # ----------------------------------------------------------
    on_refresh = refresh_TA
    on_process = prev_ROI_TA
    on_next = next_ROI_TA
    on_fit = next_and_plot_TA
